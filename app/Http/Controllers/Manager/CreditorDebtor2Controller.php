...
class CreditorDebtor2Controller extends Controller
{
    private $_model;
...
//            dd(checkRequestIsWorkingOrNot());
            ...
            $items = $this->_model->where('type',CDs::DEBTOR)->when($search, function ($query) use ($search) {
        ...
        return view('manager.cds2.index', compact('title', 'validator'));
    }
    ...
    public function create()
    {
        ...
        $validator = JsValidator::make($this->validationRules, $this->validationMessages);
        return view('manager.cds2.edit', compact('title', 'validator', 'users'));
    }
    ...
    public function edit($id)
    {
        ...
        $users = User::get();
        $validator = JsValidator::make($this->validationRules, $this->validationMessages);
        return view('manager.cds2.edit', compact('title', 'validator', 'cds', 'users'));
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->expense_id)) {
            return redirect()->route('manager.cds2.index')->with('m-class', 'success')->with('message', t('Successfully Updated'));
        } else {
            return redirect()->route('manager.cds2.index')->with('m-class', 'success')->with('message', t('Successfully Created'));
        }
    }

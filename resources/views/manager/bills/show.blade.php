...
@section('style')
    <link href="{{ asset('assets/css/demo6/pages/general/invoices/invoice-1.'.direction('.').'css') }}" rel="stylesheet" type="text/css" />
@endsection
@section('content')
    @push('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route('manager.bills.index') }}">{{ t('Bills') }}</a>
        </li>
        <li class="breadcrumb-item">
            {{ t('Bill Review') }}
        </li>
                    ...
                    <div class="kt-invoice__head" style="background: linear-gradient(to right,#db1515,#ec5252);">
                        <div class="kt-invoice__container kt-invoice__container--centered">
                            <div class="kt-invoice__logo mb-3" style="padding-top: 2rem;">
                                <a href="{{route('manager.bills.show', $bill->id)}}">
...
{{--                                    <span class="text-white" style="font-weight: 500">{{ t('User :').' '.optional($bill->client)->name }}</span>--}}
                            ...
                            <div class="kt-invoice__items" style="border-top: 1.5px solid #ffffff;padding: 2rem 0 2rem 0;">
                                <div class="kt-invoice__item">
                                    <span class="kt-invoice__subtitle">{{ t('Bill') .' '. t('Date') }} :</span>
                                    <span class="kt-invoice__text text-white" style="font-weight: 500" dir="ltr">{{ \Carbon\Carbon::parse($bill->created_at)->format(DATE_FORMAT) }}</span>
                                </div>
                                <div class="kt-invoice__item">
                                    <span class="kt-invoice__subtitle">{{ t('ORDER NO') }}</span>
                                    <span class="kt-invoice__text text-white" style="font-weight: 500 " dir="ltr"> {{ $bill->uuid }}</span>
                                </div>
...
{{--                                        <td class="text-center">{{ $item/*->pivot*/->cost_before }}</td>--}}
                        ...
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tr>
                                    <th>{{ t('Client') }}</th>
                                    <th class="text-center">{{ t('Amount') }}</th>
                                    <th class="text-center">{{ t('Uuid') }}</th>
                                    <th class="text-center">{{ t('Note') }}</th>
                                    <th class="text-center">{{ t('Created At') }}</th>
                                </tr>
                                </thead>
                                <tbody>
                                @forelse($bill->payments as $item)
                                    <tr>
                                        <td>{{ optional(optional($item->bill)->client)->name }} </td>
                                        <td class="text-center">{{ $item->amount}}</td>
                                        <td class="text-center">{{ optional($item->bill)->uuid}}</td>
                        ...
                        <br>
                        <hr>
                        <label for="">{{t('Bill Files')}}</label>
                        <br>
                        @if($bill->files()->count() > 0)

                        @foreach($bill->files as $file)
                            <a href="{{$file->file}}" target="_blank">{{t('Browse')}}</a>
                            @endforeach
                            @else
                            <label for="">{{t('No Files')}}</label>
                                @endif
                        @if($bill->note)
                            <hr/>
                            <div class="kt-invoice__content mt-4">
                                <h5 class="kt-invoice__price">{{t('Note Bill')}}:</h5>
                                <p>{{$bill->note}}</p>
                            </div>
                        @endif
    ...
    <div class="kt-invoice__actions row">
        <div class="kt-invoice__container  col-md-6">
            <button type="button" class="btn btn-danger btn-bold" onclick="window.print();">{{ t('Print Order') }}</button>
        </div>
    </div>

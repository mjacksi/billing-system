...
@section('content')
    @push('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route_admin('cds2.index') }}">{{t('cds2')}}</a>
        </li>
        <li class="breadcrumb-item">
            {{ isset($cds) ? t('Edit cds2') : t('Add cds2') }}
        </li>
    @endpush
    <div class="row">
        <div class="col-xl-12">
            <div class="kt-portlet">
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">{{ isset($cds) ? t('Edit cds2') : t('Add cds2') }}</h3>
                    </div>
                </div>
                <form enctype="multipart/form-data"
                      id="form_information"
                      class="kt-form kt-form--label-right"
                      action="{{route_admin('cds2.store') }}" method="post">
                    {{ csrf_field() }}
                    @if(isset($cds))

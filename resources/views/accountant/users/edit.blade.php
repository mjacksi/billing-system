...
@section('content')
    @push('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route('accountant.users.index') }}">{{t('users')}}</a>
        </li>
        <li class="breadcrumb-item">
            {{ isset($users) ? t('Edit users') : t('Add users') }}
        </li>
    @endpush
    <div class="row">
        <div class="col-xl-12">
            <div class="kt-portlet">
                <div class="kt-portlet__head">
                    <div class="kt-portlet__head-label">
                        <h3 class="kt-portlet__head-title">{{ isset($users) ? t('Edit users') : t('Add users') }}</h3>
                    </div>
                </div>
                <form enctype="multipart/form-data" id="form_information" class="kt-form kt-form--label-right"
                      action="{{route('accountant.users.store') }}" method="post">
                    {{ csrf_field() }}
                    @if(isset($users))
...
{{--                                            <label>--}}
{{--                                            <input type="checkbox" value="1"  {{ isset($users) && $users->draft == 1 ? 'checked' :'' }} name="draft">--}}
{{--                                            <span></span>--}}
{{--                                            </label>--}}
{{--                                        </span>--}}
{{--                                    </div>--}}
                        ...
                        <div class="kt-form__actions">
                            <div class="row">
                                <div class="col-lg-12 text-right">
                                    <button type="submit"
                                            class="btn btn-danger">{{ isset($users) ? t('Update'):t('Create') }}</button>&nbsp;
                                </div>
                            </div>
                        </div>
                    </div>
